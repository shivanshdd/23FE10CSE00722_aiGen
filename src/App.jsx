...
export default function App() {
  ...
  // SAVE
  useEffect(() => { localStorage.setItem("savedNews",    JSON.stringify(savedNews));    }, [savedNews]);
  useEffect(() => { localStorage.setItem("chatHistory",  JSON.stringify(chatHistory));  }, [chatHistory]);
  useEffect(() => { localStorage.setItem("selectedNews", JSON.stringify(selectedNews)); }, [selectedNews]);
  ...
  // FETCH NEWS via Cloudflare Worker
  const fetchNews = async () => {
    setLoading(true);
    try {
      const [enRes, hiRes] = await Promise.all([
        fetch(`${WORKER_URL}?q=${encodeURIComponent(query)}&lang=en&max=10`),
        fetch(`${WORKER_URL}?q=${encodeURIComponent(query)}&lang=hi&max=10`),
      ]);
  ...
  const toggleBookmark = (article) => {
    ...
    setSavedNews(exists
      ? savedNews.filter((a) => a.title !== article.title)
      : [...savedNews, article]
    );
  };
  ...
  const toggleSelect = (article) => {
    ...
    setSelectedNews(exists
      ? selectedNews.filter((a) => a.title !== article.title)
      : [...selectedNews, article]
    );
  };

  // AI CHAT via Ollama (local) — replace this section if you add Gemini later
  const askAI = async () => {
    if (!question) return;
    ...
    try {
      ...
      setChatHistory((prev) => {
        const updated = [...prev];
        updated[updated.length - 1].content = "⚠️ AI unavailable. Select news articles and try again.";
        return updated;
      });
    }
  };
        ...
        {sortedArticles.slice(0, visibleCount).map((a, i) => {
          ...
          return (
            <motion.div key={i} className="card">
              <h3>{a.title}</h3>
              <p>{a.description}</p>
              <small>📍 {a.source.name} • 🕒 {timeAgo(a.publishedAt)}</small>
